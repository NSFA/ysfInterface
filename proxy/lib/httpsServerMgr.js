    ...
    function prepareServer(callback) {
      ...
      co(util.getFreePort)
        .then(co.wrap(function* (port) {
          instancePort = port;
          const httpsServer = yield createHttpsServer({
            port,
          ...
          httpsServer.on('upgrade', (req, socket, head) => {
            ...
            logUtil.printLog(`wss:// is not supported when intercepting https. This request will be closed . You may either exclude this domain in your rule file, or stop all https intercepting. (${reqHost})`, logUtil.T_ERR);
            socket.end();
          });
          const result = {
